...
class MonteCarloTreeSearchNode:
    ...
    def expand(self):
        ...
        child_node = MonteCarloTreeSearchNode(new_game, self)
        self.children.append(child_node)
        return child_node
    
    def untried_moves(self):
        all_moves = self.game.get_valid_moves()
        tried_moves = [child.game.last_move for child in self.children]
...
def get_best_move(game, simulations=1000):
    root = MonteCarloTreeSearchNode(game)
    
    for _ in range(simulations):
        selected_node = root
        while not selected_node.is_fully_expanded() and selected_node.children:
            selected_node = selected_node.select_child()
        
        if not selected_node.is_fully_expanded() and not selected_node.children:
